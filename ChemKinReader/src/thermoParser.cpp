...
bool IO::ThermoParser::parseNASASection(string l1, string l2, string l3, string l4) {
    ...
    thermo.setNote(trimCopy(l1.substr(18, 6)));
    thermo.setPhase(l1.substr(44, 1));
    thermo.setTLow(from_string<double>(trimCopy(l1.substr(45, 10))));
    thermo.setTHigh(from_string<double>(trimCopy(l1.substr(55, 10))));
    ...
    {
        thermo.setTCommon(from_string<double>(trimCopy(l1.substr(65, 8))));
    }
    string elements_string = convertToCaps(l1.substr(24, 20));
    thermo.setElements(parseElements(elements_string));
    // line 2, 3 4

    double ah1 = from_string<double>(eraseAll(l2.substr(0, 15)," "));
    double ah2 = from_string<double>(eraseAll(l2.substr(15, 15)," "));
    double ah3 = from_string<double>(eraseAll(l2.substr(30, 15)," "));
    double ah4 = from_string<double>(eraseAll(l2.substr(45, 15)," "));
    double ah5 = from_string<double>(eraseAll(l2.substr(60, 15)," "));
    double ah6 = from_string<double>(eraseAll(l3.substr(0, 15)," "));
    double ah7 = from_string<double>(eraseAll(l3.substr(15, 15)," "));
    double al1 = from_string<double>(eraseAll(l3.substr(30, 15)," "));
    double al2 = from_string<double>(eraseAll(l3.substr(45, 15)," "));
    double al3 = from_string<double>(eraseAll(l3.substr(60, 15)," "));
    double al4 = from_string<double>(eraseAll(l4.substr(0, 15)," "));
    double al5 = from_string<double>(eraseAll(l4.substr(15, 15)," "));
    double al6 = from_string<double>(eraseAll(l4.substr(30, 15)," "));
    double al7 = from_string<double>(eraseAll(l4.substr(45, 15)," "));
...
map<string, int> IO::ThermoParser::parseElements(string elements_string) {
    ...
    if (elements_string.length() % 5 == 0) {
        for (unsigned int i = 0; i < elements_string.length() / 5; i++) {
            string elem = trimCopy(elements_string.substr(i * 5, 3));
            string count_str = trimCopy(elements_string.substr(3 + i * 5, 2));
