...
const regex IO::ReactionParser::TROE_center
(
    "(TROECENTER)\\s*\\/\\s*(.*?)\\s+(.*?)\\s+(.*?)(?:|\\s+(.*?))\\s*\\/"
);
...
{
    IO::splitString(reactionString, reactionStringLines_, "\n|\r\n");
...
void IO::ReactionParser::parse(vector<IO::Reaction>& reactions)
{
    ...
    for (size_t i=0; i<reactionStringLines_.size(); ++i)
    {
        ...
        sregex_iterator thirbodystart(reactionStringLines_[i].begin(), reactionStringLines_[i].end(), pattern);
        sregex_iterator thirbodyend;

        // Итераторы для поиска совпадений
        sregex_iterator iter(reactionStringLines_[i].begin(), reactionStringLines_[i].end(), pattern);
        sregex_iterator end_thrid;

        // Вывод результатов
        while (iter != end_thrid) {
            smatch match = *iter;

            if (match.str(1) != "M" && checkForPressureDependentReaction(reactionStringLines_[i]))
            {
                reaction.setThirdBodies(multimap<std::string, double> { {"!M", 0}, {match.str(1), 1}});
            }
            ++iter;
        }
        ...
        if (regex_search(start, end, what, reactionSingleRegex))
        {
            ...
            while (i < reactionStringLines_.size()-1)
            {
                ...
                else if (reaction.hasThirdBody() || reaction.isPressureDependent())
                {
                    ...
                    if (lineType == "TROECENTER")
                    {
                        reaction.setTROE_center(parseLOWTROEREV(reactionStringLines_[i + 1], TROE_center));
                        ++i;
                    }
                }
                else
...
IO::ReactionParser::parseThirdBodySpecies(const string& thirdBodies)
{
    ...
    string trim_copymed = trimCopy(thirdBodies);
    ...
    {
        string name = *j++;
        string trim_copyName = trimCopy(name);
...
IO::ReactionParser::findLineType(const string& line)
{
    ...
    if (regex_search(start, end, SRI))
        return "SRI";
    if (regex_search(start, end, TROE_center))
        return "TROECENTER";
    return "THIRDBODY";
