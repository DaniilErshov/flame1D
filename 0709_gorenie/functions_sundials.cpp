...
int InitialData(int& Nx, vector<double>& x_vect, vector<double>& T_vect, vector<double>& Y_vect, double& M, double Tstart, double Tfinish, double* Ystart, double* Yend)
{
    ...
    for (int i_cell = 0; i_cell < 3; i_cell++) {
        for (int k = num_gas_species - 1; k >=0; k--) {
            Y_vect.insert(Y_vect.begin(), Ystart[k]);
        }
        T_vect.insert(T_vect.begin(), T_vect[0]);
        x_vect.insert(x_vect.begin(), x_vect[0] - 1.5 * (x_vect[1] - x_vect[0]));
        Nx++;
        i_center++;
    }
    ...
    for (int i_cell = 0; i_cell < 3; i_cell++) {
        for (int k = 0; k < num_gas_species; k++) {
            Y_vect.push_back(Yend[k]);
        }

        T_vect.push_back(T_vect[Nx - 1]);
        x_vect.push_back(x_vect[Nx - 1] + 1.5 * (x_vect[Nx - 1] - x_vect[Nx - 2]));
        Nx++;
    }
    ...
    double Tprev, double T, double Tnext, double xprev, double x, double xnext, int number_cell)
{
    ...
    return -(2. / (h + h_left)) *
        (lambda_right * (Tnext - T) / h
            - lambda_left * (T - Tprev) / h_left)
        + Cp * M_global * dTdx + slag_chem + slag_diff;
    ...
    double Tprev, double T, double Tnext, double xprev, double x, double xnext, int i)
{
    ...
    rhoYkVk_r = rho_r * (YkVk_r[k_spec] + Y_tmp_r[k_spec] * Vc_r);
    //cout << "data->YkVk[k_spec] =  " << data->YkVk[k_spec] << "\n";


    rhoYkVk_l = rho_l * (YkVk_l[k_spec] + Y_tmp_l[k_spec] * Vc_l);
    //cout << "\n\n";
    ...
    //cout << "slag_chem =  " << slag_chem << "\n";
    //cout << "slag_diff =  " << slag_diff << "\n";
   // double dYdx = (h_left / h / (h + h_left) * Yinext[k_spec] + (h - h_left) / h / h_left * Yi[k_spec] - h / h_left / (h + h_left) * Yiprev[k_spec]);
    double dYdx = (Yi[k_spec] - Yiprev[k_spec]) / (h_left);

    return M_global * dYdx
        + slag_chem + slag_diff;
}
...
int integrate_All_IDA(int N_x, vector<double>& x_vect,
    ...
    for (int i = 1; i < N_x - 1; i++) {
        ...
        find_diff_slag(data, T_vect[i], T_vect[i + 1], Yi, Yinext, Xi, Xinext, YkVk_r, Y_tmp_r, X_tmp_r, gradX_r, rho_r, Vc_r, i, 'r');

        find_diff_slag(data, T_vect[i - 1], T_vect[i], Yiprev, Yi, Xiprev, Xi, YkVk_l, Y_tmp_l, X_tmp_l, gradX_l,rho_l, Vc_l, i - 1, 'l');
...
#pragma omp critical
    ...
    while (iout < ida_steps && M_rtol > M_rtol_need) {
...
#pragma omp critical
        ...
        //if (iout % 100 == 0)
        //    Write_to_file("detail/IDA_" + to_string(N_x) + "_" + to_string(tout), fout, x_vect,
...
static int func_All_IDA(realtype tres, N_Vector yy, N_Vector yp, N_Vector rr, void* user_data)
{
    ...
    double T_left = data->Tl;
    double T_centel_local = data->T_center;


    //data->M = yval[data->N_centr + data->NEQ_Y - 1];
    double tstart = omp_get_wtime();
#pragma omp parallel for num_threads(num_threads_global) schedule(dynamic)
    for (int i = 1; i < myNx - 1; i++) {
        double T_curr, T_prev, T_next;
        int i_T = i * (num_gas_species)+i - 1;
        int i_temp = (i - 1) * (num_gas_species + 1);
        ...
        find_diff_slag(data, T_curr, T_next, Yi, Yinext, Xi, Xinext, YkVk_r, Y_tmp_r, X_tmp_r, gradX_r, rho_r, Vc_r, i, 'r');

        find_diff_slag(data, T_prev, T_curr, Yiprev, Yi, Xiprev, Xi, YkVk_l, Y_tmp_l, X_tmp_l, gradX_l, rho_l, Vc_l, i - 1, 'l');

        double rho = get_rho(Yi, T_curr);
        double sumY = 0;
        for (int k_spec = 0; k_spec < num_gas_species; k_spec++) {
...
static int func_kinsol(N_Vector u, N_Vector f, void* user_data)
{
    ...
    if (need_update_koeffs == 1 && flag_use_save_koeffs == 1) {
        updateKoeffs(yval, data);
        cout << "updateKoeffs(yval, data);\n\n\n\n";
        cout << "update_koeffs = " << need_update_koeffs << "\n\n\n\n";
    }
#pragma omp parallel for num_threads(num_threads_global) schedule(dynamic)
    for (int i = 1; i < myNx - 1; i++) {
        double T_curr, T_prev, T_next;

        int i_T = i * (num_gas_species + 2) - 2;
        M_global = yval[i * (num_gas_species + 2) - 1];
        int i_temp = (i - 1) * (num_gas_species + 2);
        //cout << "in func i = " << i << "\n";
        //cout << "M = " << data->M << "\n\n";
        if (i == myNx - 2) {
            T_curr = yval[i_T];
        ...
        if (i == myNx - 2) {
            for (int k_spec = 0; k_spec < num_gas_species; k_spec++) {
                rval[i_temp] = Yi[k_spec] - Yiprev[k_spec];
        ...
        if (i < Ncentr) {
            rval[i_temp] = (yval[i_temp] - yval[i_temp + (num_gas_species + 2)]) / (x_vect[i + 1] - x_vect[i]);
        }
        i_temp++;
    }
    return 0;
...
static int func_final_state_kinsol(N_Vector u, N_Vector f, void* user_data)
{
    ...
    double rho = get_rho(Yi, Temp);
    for (j = 0; j < num_gas_species; j++) {
        rval[j] = ydot[j] * phyc.mol_weight[j];
    }
    double sum = 0;
...
int integrate_All_IDA_M(int N_x, vector<double>& x_vect,
    ...
    double T_left = T_vect[N_center - 1];
    double T_center = T_vect[N_center];
    double T_right = T_vect[N_center + 1];
    for (int k_spec = 0; k_spec < num_gas_species; k_spec++) {
        Yiprev[k_spec] = Y_vect[(N_center - 1) * num_gas_species + k_spec];
        Yi[k_spec] = Y_vect[N_center * num_gas_species + k_spec];
        Yinext[k_spec] = Y_vect[(N_center + 1) * num_gas_species + k_spec];
    }
    Get_molar_cons(Xi, Yi, T_center);

    chem_vel(Sn, Hn, forward_arr, reverse_arr, equilib_arr,
        T_center, Xi, ydot, N_center);


    Get_mole_fr(Xiprev, Yiprev); Get_mole_fr(Xi, Yi); Get_mole_fr(Xinext, Yinext);

    M = get_M(T_left, T_center, T_right,
        x_vect[N_center - 1], x_vect[N_center], x_vect[N_center + 1], N_center);
    ...
    for (int i = 1; i < N_x - 1; i++) {
        for (int k_spec = 0; k_spec < num_gas_species; k_spec++) {
            consval[i_temp] = 0.0;   /*constraint*/
            yval[i_temp] = Y_vect[i * num_gas_species + k_spec] + eps;
            atval[i_temp] = pow(10, -8);
            //cout << "Yi = " << i_temp << "   = " << yval[i_temp] << endl;
            i_temp++;
        }
        yval[i_temp] = T_vect[i];
        consval[i_temp] = 1.0;
        atval[i_temp] = pow(10, -1);
        //cout << "Ti = " << i_temp << "   = " << yval[i_temp] << endl << endl;
        i_temp++;
        yval[i_temp] = M;
    ...
    for (int i = 1; i < N_x - 1; i++) {
        ...
        for (int k_spec = 0; k_spec < num_gas_species; k_spec++) {
            ypval[i_temp] = -F_rightY(data, k_spec,
                T_prev, T_curr, T_next,
                x_vect[i - 1], x_vect[i], x_vect[i + 1], i) / rho;
           // cout << "i_temp Y= " << i_temp << " = " << ypval[i_temp] << "\n";
            //cout << "Y= " << i_temp << " = " << yval[i_temp] << "\n";
            i_temp++;
        }
        double Cp = get_Cp(num_gas_species, Yi, T_curr, i);
        ...
        //cout << "i_temp M = " << i_temp << " = " << ypval[i_temp] << "\n";
        //cout << "M = " << i_temp << " = " << yval[i_temp] << "\n";
        i_temp++;
    }
    //cout << NEQ << " = " << Ith(res_vect, NEQ + 1) << endl;
    ...
    double M_prev = 0;
    double M_rtol = 100;
    while (iout < ida_steps && M_rtol > M_rtol_needed) {
        retval = IDASolve(mem, tout, &tret, yy, yp, IDA_NORMAL);

        cout << "t = " << tout << "\n";
        if (check_retval(&retval, "IDASolve", 1)) return(1);
        M_rtol = abs(M_prev / M_global - 1.);
        M_prev = M_global;
    ...
    for (int i = 1; i < myNx - 1; i++) {
       ...
       //cout << "rvalT " << i_temp << "= " << rval[i_temp] << "\n";
        ...
        if (i == Ncentr) {
            ...
            //    T_prev, T_curr, T_next,
            //    x_vect[i - 1], x_vect[i], x_vect[i + 1], i); 
            double M_tmp = get_M(T_prev, T_curr, T_next,
                x_vect[Ncentr - 1], x_vect[Ncentr], x_vect[Ncentr + 1], Ncentr);


            rval[i_temp] = yval[i_temp] - M_tmp;
        }
        if (i > Ncentr) {
            rval[i_temp] = -(yval[i_temp] - yval[i_temp - (num_gas_species + 2)]);
        }
        if (i < Ncentr) {
            rval[i_temp] = -(yval[i_temp + (num_gas_species + 2)] - yval[i_temp]);
        }
        //cout << "yval[i_temp] = " << yval[i_temp] << "\n";
        //cout << "rvalM " << i_temp  << "= " << rval[i_temp] << "\n";
        i_temp++;
    }
    delete tcur;
    return 0;
